...
| Provider | `health`, explainable `route`, `benchmark --dry-run`, invoke, and body diagnosis are wired. | External model quality/latency requires configured provider endpoints. |
...
| Hub / MCP | Full tests pass; universal agent MCP stdio probe discovers 13 tools. | Remote registries and credentials were not exercised. |
...
| ROS Connector | Read-only ping/discover passes on ROS2 ports 9090 and 32887 and marks `/turtle1/cmd_vel` high risk. | ROS1 port 9091 has a container websocket Host/port mismatch. |
