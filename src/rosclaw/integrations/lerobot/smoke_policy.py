...
async def run_smoke_policy(options: SmokePolicyOptions) -> SmokeReport:
    """Run the full P1.1 smoke workflow and return a report."""
    ...
    if stage2.status != "ok":
        ...
        report.error = stage2.error
        return _maybe_write_report(report, options)

    # Stage 3: load-test
    stage3 = await _stage_load_test(local_policy_path, options)
    stages["load_test"] = stage3.status
    ...
    if stage3.status != "ok":
        ...
        report.policy = policy_info
        report.features = features
        report.error = stage3.error
        return _maybe_write_report(report, options)

    # Stage 4: infer
    if not options.skip_infer:
        stage4 = await _stage_infer(local_policy_path, options)
        stages["infer"] = stage4.status
        timings["infer_time_sec"] = stage4.latency_sec
        if stage4.status == "ok":
            action_proposal = stage4.data.get("action_proposal")
        if stage4.status != "ok":
            report.stages = stages
            report.runtime = runtime_info
            report.policy = policy_info
            report.features = features
            report.error = stage4.error
            return _maybe_write_report(report, options)
    else:
        stages["infer"] = "skipped"
...
def _stage_materialize(options: SmokePolicyOptions) -> SmokeStageResult:
    ...
    try:
        local_path = materialize_policy_path(
            options.policy_path,
            revision=options.revision,
    ...
    return SmokeStageResult(status="ok", data={"local_path": local_path})
...
async def _stage_infer(local_path: Path, options: SmokePolicyOptions) -> SmokeStageResult:
    ...
    # Enforce the P1 safety contract one more time at the smoke layer.
