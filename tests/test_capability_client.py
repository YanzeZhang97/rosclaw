...
async def test_capability_client_pick_up_task(client):
    ...
    assert result.status == "success", (
        f"Expected success, got {result.status!r}: steps={result.steps}, errors={result.errors}"
    )
...
async def test_capability_client_task_result_fields(client):
    ...
    assert result.status in ("success", "partial", "failed"), (
        f"Unexpected status {result.status!r}: steps={result.steps}, errors={result.errors}"
    )
...
async def test_capability_client_failed_step_aborts(client):
    ...
    assert result.status == "failed", (
        f"Expected failed, got {result.status!r}: steps={result.steps}, errors={result.errors}"
    )
