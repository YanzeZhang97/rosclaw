...
class TestListEpisodes:
    def test_empty_directory(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setattr("rosclaw.practice.cli._artifact_dir", lambda: tmp_path)
        list_episodes()
    ...
    def test_with_episodes(self, tmp_path, monkeypatch, capsys):
        ...
        meta.write_text(json.dumps({"robot_id": "ur5e", "status": "success", "reward": 0.95}))

        monkeypatch.setattr("rosclaw.practice.cli._artifact_dir", lambda: artifacts)
        list_episodes()
        captured = capsys.readouterr()
...
class TestReplayEpisode:
    def test_full_replay(self, tmp_path, monkeypatch, capsys):
        ...
        traj.write_text(json.dumps({"phase": "approach", "skill_name": "reach"}) + "\n")
